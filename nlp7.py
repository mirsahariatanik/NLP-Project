...
def generate_event(event_set,participant_set,week_set,day_set,hour_set,ampm_set):
    ...
    #print(e[0])
    if (hour=='12'):
        event_generated = 'schedule' + ' ' + week + ' ' + day + ' ' +event + ' ' + 'with'  + ' ' + participant +' '+ 'from' + ' ' + hour+ampm+' '+'to'+ ' '+ '2'+ampm
        if (len(e)==1):
            eve= 'schedule:O'+' '+'schedule:O'+' '+week+':B-date' +' '+ day+':I-date' + ' '+event+ ':B-title'+ ' '+'with' + ':I-title'+' '+participant+':I-title'+' '+ 'from'+':O'+' '+ hour+ampm+':B-start-time'+' '+'to'+':O'+' '+ '2'+ampm+':B-end-time'
        else:
            eve= 'schedule:O'+' '+'schedule:O'+' '+week+':B-date' +' '+ day+':I-date' + ' '+e[0]+ ':B-title'+ ' '+e[1]+ ':I-title'+' '+'with' + ':I-title'+' '+participant+':I-title'+' '+'from'+':O'+' '+ hour+ampm+':B-start-time'+' '+'to'+':O'+' '+ '2'+ampm+':B-end-time'
        ...
        event_generated = 'schedule' + ' ' + week + ' ' + day + ' ' +event + ' ' + 'with'  + ' ' + participant +' '+ 'from' + ' ' + hour+ampm+' '+'to'+ ' '+ str(int(hour)+2)+ampm
        if (len(e)==1):
            eve= 'schedule:O'+' '+'schedule:O'+' '+week+':B-date' +' '+ day+':I-date' + ' '+event+ ':B-title'+ ' '+'with' + ':I-title'+' '+participant+':I-title'+' '+ 'from'+':O'+' '+ hour+ampm+':B-start-time'+' '+'to'+':O'+' '+ str(int(hour)+2)+ampm+':B-end-time'
        else:
            eve= 'schedule:O'+' '+'schedule:O'+' '+week+':B-date' +' '+ day+':I-date' + ' '+e[0]+ ':B-title'+ ' '+e[1]+ ':I-title'+' '+'with' + ':I-title'+' '+participant+':I-title'+' '+'from'+':O'+' '+ hour+ampm+':B-start-time'+' '+'to'+':O'+' '+ str(int(hour)+2)+ampm+':B-end-time'
        #find_taxi ( taxi-destination = backstreet bistro ; taxi-arriveby = 19:30 )
